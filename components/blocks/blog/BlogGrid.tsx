...
type BlogGridProps = {
  ...
  /** Set on a location hub - scopes results to that location's posts. */
  location?: string;
};

export default function BlogGrid({ categories, searchParams, location }: BlogGridProps) {
  const query = searchParams.q;
  const category = searchParams.category;
  const page = searchParams.page ? parseInt(searchParams.page, 10) : 1;

  // Build a key that changes whenever parameters change to re-trigger Suspense
  const suspenseKey = `${location ?? ""}-${query}-${category}-${page}`;
  ...
  return (
    <section className="w-full bg-background px-4 sm:px-5 md:px-8 py-[var(--space-xl)] transition-colors duration-300">
      ...
      <Suspense key={suspenseKey} fallback={<BlogResultsSkeleton />}>
        <BlogResults query={query} category={category} page={page} location={location} />
      </Suspense>
    </section>
  );
}
