...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    if (desktopVideoRef.current) {
      desktopVideoRef.current
        .play()
        .catch((e) => console.log("Desktop video autoplay error:", e));
    }
    if (mobileVideoRef.current) {
      mobileVideoRef.current
        .play()
        .catch((e) => console.log("Mobile video autoplay error:", e));
    }
  ...
  return (
    ...
    >
        ...
        <div className="flex-none flex items-center justify-center">
          ...
          <div className="hidden min-[1100px]:flex items-center justify-center gap-[var(--gap-lg)] max-[1300px]:gap-[12px] whitespace-nowrap">
            {navLinks.map((link) => {
              ...
              const underline = (
                <span
                  className={`absolute -bottom-2 left-0 w-full h-[2.5px] bg-primary transition-transform duration-300 origin-left ${isActive ? "scale-x-100" : "scale-x-0 group-hover:scale-x-100"}`}
                ></span>
              );
        ...
        <div className="flex-1 basis-0 min-w-0 flex items-center justify-end gap-[var(--gap-md)] max-[1300px]:gap-[10px] z-10">
          ...
          >
            {mounted ? (
              theme === "dark" ? (
                <Sun
                  size={19}
                  className="max-[1300px]:w-[14px] max-[1300px]:h-[14px] min-[1440px]:w-[22px] min-[1440px]:h-[22px]"
                />
              ) : (
                <Moon
                  size={19}
          ...
          */}
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="hidden min-[1100px]:block relative text-white hover:text-primary transition-colors"
          >
            <img
          ...
          */}
          ...
          {/* Mobile Menu Toggle */}
          <button
            className={`min-[1100px]:hidden absolute right-4 top-1/2 -translate-y-1/2 text-white p-2 z-50 transition-all active:scale-95 ${
          ...
          >
            <Menu
              className="w-7 h-7 max-[500px]:w-[23px] max-[500px]:h-[23px]"
              strokeWidth={2.5}
            />
          </button>
        </div>
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`min-[1100px]:hidden fixed inset-0 bg-black transition-opacity duration-300 z-[9999] h-full ${
          isMobileMenuOpen
      ...
      >
        ...
        >
          <X
            className="w-8 h-8 max-[500px]:w-[27px] max-[500px]:h-[27px]"
            strokeWidth={2.5}
          />
        </button>
          ...
          >
            ...
            >
              ...
              <img
                src="/site-logo/FJH-logo-white.png"
                alt="Flame Japanese Hibachi"
                className="w-[200px] h-[80px] max-[500px]:w-[170px] max-[500px]:h-[68px] object-contain"
              />
            </video>
          </Link>
          ...
          <div className="flex flex-col items-center gap-8 max-[500px]:gap-[27px] w-full pb-10">
            ...
            <div className="flex items-center gap-12 max-[500px]:gap-10">
              ...
              >
                {mounted ? (
                  theme === "dark" ? (
                    <Sun className="w-7 h-7 max-[500px]:w-[23px] max-[500px]:h-[23px]" />
                  ) : (
                    <Moon className="w-7 h-7 max-[500px]:w-[23px] max-[500px]:h-[23px]" />
                  )
                ) : (
